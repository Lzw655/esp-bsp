...
static uint32_t mipi_csi_disp_hsize = 0;
static uint32_t mipi_csi_disp_vsize = 0;
...
static esp_err_t isp_clock_init(void)
{
...
{
    printf("Initialize ISP\n");

    isp_clock_init();

    printf("ISP_EN: %d, hor_res: %d, ver_res: %d\n", ISP.cntl.en, hor_res, ver_res);

    mipi_csi_disp_hsize = hor_res;
    mipi_csi_disp_vsize = ver_res;
    ...
    // xTaskCreatePinnedToCore(isp_task, "isp_task", 8192, NULL, 5, NULL, 0);
    return ESP_OK;
}
