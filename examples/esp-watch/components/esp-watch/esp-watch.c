...
#include "priv_include/bsp_lcd_panel_st7789.h"
#include "bsp/esp-watch.h"
...
static const char *TAG = "ESP-WATCH";
...
static lv_disp_t *lvgl_port_display_init(void)
{
    ...
    BSP_ERROR_CHECK_RETURN_NULL(bsp_lcd_new_panel_st7789(io_handle, &panel_config, &panel_handle));
...
static esp_err_t lvgl_port_indev_init(void)
{
    ...
    BSP_ERROR_CHECK_RETURN_ERR(esp_lcd_touch_new_i2c_cst816s(tp_io_handle, &tp_cfg, &tp));
    assert(tp);
