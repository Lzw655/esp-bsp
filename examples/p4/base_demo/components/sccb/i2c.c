...
#define TEST_WITH_OS 0
#define TEST_INTERRUPT       (0)

static int i2c_num = 0;
#define I2C_INTR_SOURCE     (((INTERRUPT_CORE0_I2C0_INT_MAP_REG - DR_REG_INTERRUPT_CORE0_BASE) / 4) + i2c_num)
...
static void delay_us(uint32_t t)
{
    // for (uint32_t tu = 0; tu < 1000*t; tu++);
    ets_delay_us(t);
}
#endif
...
static void i2c_set_pin(int io_scl, int io_sda, int od_en)
{
    ...
    for (int x = 0; x < 2; x++) {
        ...
        GPIO.func_out_sel_cfg[io_num[x]].oe_sel = 1;
...
void i2c_init(int rate, int io_scl, int io_sda)
{
    ...
    int sclk_div_num = 4 - 1;

    REG_SET_FIELD(HP_SYS_CLKRST_PERI_CLK_CTRL10_REG, HP_SYS_CLKRST_REG_I2C0_CLK_DIV_NUM, sclk_div_num);
    REG_SET_FIELD(HP_SYS_CLKRST_PERI_CLK_CTRL10_REG, HP_SYS_CLKRST_REG_I2C0_CLK_DIV_NUMERATOR, 0);
    REG_SET_FIELD(HP_SYS_CLKRST_PERI_CLK_CTRL10_REG, HP_SYS_CLKRST_REG_I2C0_CLK_DIV_DENOMINATOR, 0);
    REG_SET_FIELD(HP_SYS_CLKRST_PERI_CLK_CTRL10_REG, HP_SYS_CLKRST_REG_I2C0_CLK_SRC_SEL, 0);
    REG_CLR_BIT(HP_SYS_CLKRST_SOC_CLK_CTRL2_REG, HP_SYS_CLKRST_REG_I2C0_APB_CLK_EN);
    REG_SET_BIT(HP_SYS_CLKRST_SOC_CLK_CTRL2_REG, HP_SYS_CLKRST_REG_I2C0_APB_CLK_EN);
    REG_CLR_BIT(HP_SYS_CLKRST_PERI_CLK_CTRL10_REG, HP_SYS_CLKRST_REG_I2C0_CLK_EN);
    REG_SET_BIT(HP_SYS_CLKRST_PERI_CLK_CTRL10_REG, HP_SYS_CLKRST_REG_I2C0_CLK_EN);
    REG_SET_BIT(HP_SYS_CLKRST_HP_RST_EN1_REG, HP_SYS_CLKRST_REG_RST_EN_I2C0);
    REG_CLR_BIT(HP_SYS_CLKRST_HP_RST_EN1_REG, HP_SYS_CLKRST_REG_RST_EN_I2C0);

    printf("i2c date: 0x%x\n", (int)I2C0.date);

    uint32_t half_cycle = (40000000 / (sclk_div_num + 1)) / rate / 2; 
