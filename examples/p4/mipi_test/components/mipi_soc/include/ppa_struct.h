...
typedef volatile struct {
    ...
    union {
        struct {
            uint32_t sr_rx_cm                      :    4;  /*The source image color mode for Scaling and Rotating engine Rx. 0: ARGB8888. 1: RGB888. 2: RGB565. 8: YUV420. others: Reserved.*/
            uint32_t sr_tx_cm                      :    4;  /*The destination image color mode for Scaling and Rotating engine Tx. 0: ARGB8888. 1: RGB888. 2: RGB565. 8: YUV420. others: Reserved.*/
    ...
    union {
        struct {
            ...
            uint32_t x_yuv420_rx_scale_err_st      :    1;  /*The error is that the ha/hb/x param in dma2d descriptor is an odd num when enable yuv420 rx*/
            uint32_t y_yuv420_rx_scale_err_st      :    1;  /*The error is that the va/vb/y param in dma2d descriptor is an odd num when enable yuv420 rx*/
            uint32_t x_yuv420_tx_scale_err_st      :    1;  /*The error is that the ha/hb/x param in dma2d descriptor is an odd num when enable yuv420 tx*/
            uint32_t y_yuv420_tx_scale_err_st      :    1;  /*The error is that the va/vb/y param in dma2d descriptor is an odd num when enable yuv420 tx*/
